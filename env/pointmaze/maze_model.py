...
def point_maze(maze_str, density_scale=1.0, damping_scale=1.0):
    ...
    default.joint(damping=1.0 * float(damping_scale), limited='false')
    default.geom(friction=".5 .1 .1", density=str(1000.0 * float(density_scale)),
                 margin="0.002", condim="1", contype="2", conaffinity="1")
...
class MazeEnv(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    def __init__(self,
        ...
        model = point_maze(maze_spec, density_scale=density_scale, damping_scale=damping_scale)
